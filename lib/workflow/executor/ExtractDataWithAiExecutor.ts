  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a webscraper helper that extracts data from HTML or text. You will be given a piece of text or HTML content 
            as input and also the prompt with the data you have want to extract. The response should always be only the extracted data 
    ...
    environment.log.info(
      `Completition token: ${response.usage?.completion_tokens}`,
    );
